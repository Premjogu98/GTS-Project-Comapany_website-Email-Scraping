...
def Collect_Link():
    ...
    while a == 0:
        try:
            ...
            mycursorLocal = mydb_Local.cursor()
            Links_List = []
            mycursorLocal.execute("SELECT Company_Name FROM mcagovin_orgnames WHERE website_status = '' LIMIT 10000,10000")
...
def Nav_Links(Links_List):
    ...
    for Company_name in Links_List:
        ...
        while a == 0:
            try:
                Company_name = Company_name.replace("'", "''")  # Most Important When inserting Company Name
